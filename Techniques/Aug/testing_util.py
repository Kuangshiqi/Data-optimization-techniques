...
def run_test(in_outs, code: str = None, debug: bool = False, timeout=4):
    """
    ...
    if code is None:
        ...
        if which_type == CODE_TYPE.call_based:
            ...
            sys.stdout = io.StringIO()  # 将 stdout 重定向到 StringIO 对象
            try:
                tmp_sol = RuntimeModule.from_string("tmp_sol", "", sol)
                captured_output = sys.stdout.getvalue()  # 捕获输出内容（如果需要）
        ...
        for index, inputs in enumerate(in_outs["inputs"]):
            ...
            if which_type == CODE_TYPE.call_based:  # Call-based
                ...
                sys.stdout = io.StringIO()  # 重定向到 StringIO
                try:
                    # print("------------")
                    # print(inputs)
                    output = method(*inputs)
                    captured_output = sys.stdout.getvalue()  # 捕获输出内容
